...
export default function StaffListTable() {
  ...
  const fetchStaff = async () => {
    try {
      const res = await axios.get("https://rjtask-server.vercel.app/api/auth");
      setStaff(res.data);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `https://rjtask-server.vercel.app/api/auth/${id}`
      );
  ...
  try {
    await axios.put(
      `https://rjtask-server.vercel.app/api/auth/${id}/reset-password`,
      { newPassword }
    );
